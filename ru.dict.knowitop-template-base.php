<?php
/**
 * Localized data
 *
 * @copyright   Copyright (C) 2017 Vladimir Kunin <v.b.kunin@gmail.com>
 * @license     http://opensource.org/licenses/AGPL-3.0
 */

Dict::Add('RU RU', 'Russian', 'Русский', array(

    'Class:ObjectTemplate' => 'Шаблон объекта',
    'Class:ObjectTemplate+' => 'Шаблон объекта',
    'Class:ObjectTemplate/Name' => '%1$s',
    'Class:ObjectTemplate/Attribute:name' => 'Название',
    'Class:ObjectTemplate/Attribute:name+' => 'Название шаблона',
    'Class:ObjectTemplate/Attribute:description' => 'Описание',
    'Class:ObjectTemplate/Attribute:description+' => 'Описание шаблона',
    'Class:ObjectTemplate/Attribute:status' => 'Статус',
    'Class:ObjectTemplate/Attribute:status+' => 'Статус',
    'Class:ObjectTemplate/Attribute:status/Value:inactive' => 'Неактивный',
    'Class:ObjectTemplate/Attribute:status/Value:active' => 'Активный',
    'Class:ObjectTemplate/Attribute:finalclass' => 'Тип',
    'Class:ObjectTemplate/Attribute:finalclass+' => 'Тип',

    'ObjectTemplate:baseinfo' => 'Основная информация',
    'ObjectTemplate:template' => 'Шаблон',

    'Menu:ObjectTemplate' => 'Шаблоны объектов',
    'Menu:ObjectTemplate+' => 'Шаблоны объектов'
));
